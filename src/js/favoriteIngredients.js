...
const noFavIngredientsMarkup = document.querySelector('.no-fav-ingredients');
let objFavorite = {};
...
function checkIngredientFavouritePage(event) {
  if (event.target.classList.contains('card__btn')) {
    ...
    const elemFavoriteIngr =
      event.target.closest('.card__btn-add') ||
      event.target.closest('.ingredients-modal__btn');
    ...
    if (cardsSectionEl.classList.contains('is-hidden')) {
      ...
      for (const item in dataFromStorageIngr) {
        getIngredientById(dataFromStorageIngr[item]).then(data => {
          ...
          let forBtnFavoriteIngr =
            markupIngr.querySelectorAll('.card__btn-add');
...
function markupIngredientCard(dataForCard, position, activeNotActive) {
  for (const item of dataForCard) {
    const htmlCards = `<div class="favor-ingredient__card">
      ...
      <li><p class="ingredients-modal__pretitle"> ${
        item.strType ? `${item.strType}` : ''
      } </p></li>
      </ul>
      <div class="card__btns">
                                        <button type="button" class="card__btn" id = "${
                                          item.idIngredient
                                        }">Learn more</button>
                                        <button type="button" class="card__btn-add ${activeNotActive} ingr" idingr = "${
      item.idIngredient
    }" ident="${item.idDrink}">
...
//-----------------toggle between favorite cards markup and no favorite ingridient markup
function markupToggle() {
  noFavIngredientsMarkup.classList.toggle('is-hidden');

...
async function addCocktailToFav(searchParams) {
    const cocktails = {};
    ...
    try {await update(data, cocktails);
...
async function addIngrToFav(searchParams) {
    ...
    try {await update(data, ingredients);} 
    catch { alert('Please, login');}
}
...
async function getFavouriteCocktails() {
    if (auth.currentUser) {                                                                   // перевірка на логін
        const userId = auth.currentUser.uid;
        const dbRef = ref(getDatabase());
        return await get(child(dbRef, `favourite/${userId}/cocktails`))
            .then(snapshot => {
                    if (snapshot.exists()) {                                                   // перевіряємо чи є взагалі збережені коктейлі на firebase
